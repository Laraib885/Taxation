...
import { MainContractAbi, MainContractAddress } from "../Constants";
...
const PaymentContext = (props) => {
  ...
  const MainContract = async (needSigner) => {
    try {
      ...
      const contract = new Contract(MainContractAddress, MainContractAbi, signer);
      return contract;
    } catch (error) {
  ...
  return (
    <AppContext.Provider value={{ MainContract, buyUsingEther, connectWallet, address, buyUsingToken, isOpen, setIsOpen, priceInToken, currentTokenName, handleBuyUsingTokenSubmit, ipfs }}>
      {props.children}
    </AppContext.Provider>
  )
}
